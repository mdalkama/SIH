...
import { Loader2, Briefcase, IndianRupee, Calendar, CheckCircle, Info, X, AlertTriangle, Building2, BookCheck, Eye, FileText, Link, Star, BookUser, ThumbsUp, ThumbsDown } from 'lucide-react';
        ...
        {[...Array(3)].map((_, i) => (
            <div key={i} className="bg-white p-6 rounded-xl border border-slate-200">
                <div className="flex justify-between items-start">
                    <div><div className="h-6 w-48 bg-slate-200 rounded-md mb-2"></div><div className="h-5 w-32 bg-slate-200 rounded-md"></div></div>
                    <div className="h-8 w-24 bg-slate-200 rounded-full"></div>
                </div>
                <div className="flex items-center gap-6 mt-4 pt-4 border-t border-slate-100">
                    <div className="h-5 w-1/4 bg-slate-200 rounded-md"></div><div className="h-5 w-1/4 bg-slate-200 rounded-md"></div>
                </div>
            </div>
        ))}
    </div>
            ...
            <div className="bg-slate-50 rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
                <div className="p-5 border-b border-slate-200 bg-white flex justify-between items-start">
                    <div>
                        ...
                        <div className="flex items-center gap-2 text-slate-600 mt-1"><Building2 size={14} /><span className="font-semibold text-indigo-600">{drive.companyName}</span></div>
                    </div>
                ...
                <div className="p-6 overflow-y-auto">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                        <div className="flex items-center gap-3 p-4 bg-white rounded-lg border border-slate-200"><IndianRupee className="w-7 h-7 text-green-500 flex-shrink-0" /><div><p className="text-xs font-semibold text-slate-500 uppercase">Package</p><p className="font-bold text-slate-800">{drive.packageLPA} LPA</p></div></div>
                        <div className="flex items-center gap-3 p-4 bg-white rounded-lg border border-slate-200"><BookUser className="w-7 h-7 text-blue-500 flex-shrink-0" /><div><p className="text-xs font-semibold text-slate-500 uppercase">Eligible Courses</p><p className="font-bold text-slate-800">{drive.eligibleCourses.join(', ')}</p></div></div>
                        <div className="flex items-center gap-3 p-4 bg-white rounded-lg border border-slate-200"><Star className="w-7 h-7 text-amber-500 flex-shrink-0" /><div><p className="text-xs font-semibold text-slate-500 uppercase">Min. CGPA</p><p className="font-bold text-slate-800">{drive.minCGPA}</p></div></div>
                    </div>
                    <div>
                        <h3 className="font-semibold text-slate-800 mb-2 text-base">Job Description</h3>
                        <div className="prose prose-sm text-slate-600 max-w-none bg-white p-4 rounded-md border border-slate-200"><p>{drive.jobDescription}</p></div>
                    </div>
                </div>
...
const ApplicationStatusActions = ({ drive, onRespond, loadingAction }) => {
    const { applicationStatus } = drive;
    if (applicationStatus === 'SHORTLISTED') {
        const isAccepting = loadingAction && loadingAction.driveId === drive._id && loadingAction.type === 'accept';
        const isDeclining = loadingAction && loadingAction.driveId === drive._id && loadingAction.type === 'decline';
        return (
            <div className="mt-4 pt-4 border-t border-slate-100">
                <p className="text-sm font-semibold text-yellow-800 mb-2">Congratulations! You have been shortlisted.</p>
                <div className="flex flex-col sm:flex-row gap-3">
                    <button onClick={() => onRespond(drive._id, 'OFFER_ACCEPTED', 'accept')} disabled={isAccepting || isDeclining} className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-semibold text-sm disabled:bg-green-300">{isAccepting ? <Loader2 size={16} className="animate-spin"/> : <ThumbsUp size={16}/>} Accept Offer</button>
                    <button onClick={() => onRespond(drive._id, 'OFFER_DECLINED', 'decline')} disabled={isAccepting || isDeclining} className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-semibold text-sm disabled:bg-red-300">{isDeclining ? <Loader2 size={16} className="animate-spin"/> : <ThumbsDown size={16}/>} Decline Offer</button>
                </div>
            </div>
        );
    }
...
const StudentPlacementDrives = () => {
    ...
    const handleOfferResponse = async (driveId, newStatus, actionType) => {
        setActionLoading({ driveId, type: actionType });
        try {
    ...
    const renderDriveCard = (drive) => (
        <div key={drive._id} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm transition-shadow hover:shadow-md">
            ...
            {activeTab === 'applied' ? (
                <ApplicationStatusActions drive={drive} onRespond={handleOfferResponse} loadingAction={actionLoading} />
            ) : (
                <div className="mt-5 flex flex-col sm:flex-row gap-3">
                    <button onClick={() => setSelectedDrive(drive)} className="w-full sm:w-auto px-4 py-2 bg-white text-slate-700 border border-slate-200 rounded-lg hover:bg-slate-50 font-semibold text-sm flex items-center justify-center gap-2"><Eye size={16}/> View Details</button>
