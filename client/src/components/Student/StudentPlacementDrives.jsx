...
import { Loader2, Briefcase, IndianRupee, Calendar, CheckCircle, Info, X, AlertTriangle, Building2, BookCheck } from 'lucide-react';
		...
		<div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
			<div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
				<div className="p-4 border-b flex justify-between items-center">
					<div>
						<h2 className="text-lg font-bold text-slate-800">{drive.jobTitle}</h2>
				...
				<div className="p-6 overflow-y-auto">
					...
					<p className="text-sm text-slate-600 whitespace-pre-wrap">{drive.jobDescription}</p>
					<h3 className="font-semibold text-slate-700 mt-6 mb-2">Eligibility</h3>
					<div className="text-sm text-slate-600 space-y-1">
						<p><span className="font-medium">Courses:</span> {drive.eligibleCourses.join(', ')}</p>
						<p><span className="font-medium">Min CGPA:</span> {drive.minCGPA}</p>
					</div>
				</div>
				<div className="p-4 bg-slate-50 border-t flex justify-end">
					<button onClick={onClose} className="px-4 py-2 bg-slate-200 text-slate-700 rounded-lg hover:bg-slate-300 font-semibold">Close</button>
				</div>
			</div>
		</div>
	);
...
const StudentPlacementDrives = () => {
	...
	const fetchDrives = useCallback(async () => {
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(errData.message || "Failed to fetch placement drives.");
			}
	...
	const handleApply = async (driveId, companyName) => {
		...
		try {
			...
			fetchDrives(); // Refresh the list to show "Applied" status
		} catch (err) {
		...
		<div className="font-sans">
			...
			<div className="space-y-4">
				{drives.length > 0 ? drives.map(drive => (
					<div key={drive._id} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
						<div className="flex flex-col sm:flex-row justify-between sm:items-start gap-4">
							...
							<span className="flex-shrink-0 px-3 py-1 text-sm font-semibold rounded-full bg-green-100 text-green-800">Eligible</span>
						</div>
						...
						<div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 mt-4 pt-4 border-t border-slate-100">
							<div className="flex items-center gap-2 text-slate-700">
								...
								<div>
									<p className="font-semibold">{drive.packageLPA} LPA</p>
							...
							<div className="flex items-center gap-2 text-slate-700">
								...
								<div>
									<p className="font-semibold">{new Date(drive.applicationDeadline).toLocaleDateString('en-GB')}</p>
									<p className="text-xs text-slate-500">Apply Before</p>
								</div>
							</div>
						</div>

						<div className="mt-4 flex flex-col sm:flex-row gap-2">
							<button onClick={() => setSelectedDrive(drive)} className="w-full sm:w-auto px-4 py-2 bg-white text-slate-700 border border-slate-300 rounded-lg hover:bg-slate-50 font-semibold text-sm">View Details</button>
							<button
					...
					<div className="text-center py-20 px-6 bg-white rounded-xl border-2 border-dashed border-slate-200">
