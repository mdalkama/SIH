...
const FeesDashboard = () => {
    ...
    const pendingFees = useMemo(() => {
        ...
        const pendingSemesters = (paymentData.semesters || []).filter(s => s.paid < s.fees).map(s => ({ _id: s._id, _type: 'semester', type: `Semester ${s.semester} Fee`, description: `Academic Fees for Semester ${s.semester}`, pendingAmount: s.fees - s.paid, dueDate: new Date(new Date(s.createdAt).setMonth(new Date(s.createdAt).getMonth() + 2)).toISOString().split('T')[0] }));
    ...
    const renderOverview = () => {
        ...
        const allSemesters = paymentData?.semesters || [];
        const totalSemFees = allSemesters.reduce((acc, sem) => acc + (sem.fees || 0), 0);
        ...
        return (
            <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div className="bg-blue-50 p-6 rounded-lg border border-blue-200"><div><p className="text-blue-600 text-sm font-medium">Total Sem Fees</p><p className="text-2xl font-bold text-blue-700">₹{totalSemFees.toLocaleString() || 0}</p></div></div>
                </div>
                ...
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                    <h3 className="text-lg font-semibold mb-4">Semester Fee Ledger</h3>
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead className="bg-slate-50 text-left">
                                <tr>
                                    <th className="p-3 font-semibold text-slate-600">Semester</th>
                                    <th className="p-3 font-semibold text-slate-600 text-right">Total Fee</th>
                                    <th className="p-3 font-semibold text-slate-600 text-right">Amount Paid</th>
                                    <th className="p-3 font-semibold text-slate-600 text-right">Pending Amount</th>
                                    <th className="p-3 font-semibold text-slate-600 text-center">Status</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-200">
                                {allSemesters.map((sem) => {
                                    const totalFee = sem.fees; // Corrected: Use sem.fees directly
                                    const pending = totalFee - (sem.paid || 0);
                                    const isPaid = pending <= 0;

                                    return (
                                        <tr key={sem._id} className="hover:bg-slate-50">
                                            <td className="p-3 font-medium text-slate-800">Semester {sem.semester}</td>
                                            <td className="p-3 text-right text-slate-600 font-mono">₹{totalFee.toLocaleString('en-IN')}</td>
                                            <td className="p-3 text-right text-green-600 font-mono">₹{(sem.paid || 0).toLocaleString('en-IN')}</td>
                                            <td className={`p-3 text-right font-semibold font-mono ${pending > 0 ? 'text-red-600' : 'text-slate-500'}`}>₹{pending.toLocaleString('en-IN')}</td>
                                            <td className="p-3 text-center">
                                                {isPaid ? (
                                                    <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800"><CheckCircle size={12} /> Paid</span>
                                                ) : (
                                                    <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800"><Clock size={12} /> Pending</span>
                                                )}
                                            </td>
                                        </tr>
                                    );
                        ...
                        {allSemesters.length === 0 && <p className="text-center text-gray-500 py-8">No semester fee records have been assigned yet.</p>}
                    </div>
                </div>
            </div>
        );
                        ...
                        paymentData.paymentHistory.slice().reverse().map((payment) => (
                            <div key={payment._id} className="relative">
                                ...
                                <div className="ml-4">
                                    ...
                                    <div className="flex justify-end mt-3"><button onClick={() => downloadReceipt(payment.receiptNo)} className="flex items-center px-3 py-1.5 text-blue-600 border border-blue-200 bg-blue-50 rounded-lg hover:bg-blue-100 text-sm font-medium transition-colors"><Download className="w-4 h-4 mr-2" />Download Receipt</button></div>
                                </div>
                            </div>
                        )) : 
