...
const UniversityAdminManageRoles = () => {
    ...
    const [formData, setFormData] = useState({
        name: '', email: '', password: '', staffId: '', gender: '', salary: '', phone: '', collegeCode: '', status: 'Active',
    });
    ...
    useEffect(() => {
        const fetchStaff = async () => {
            setLoading(true);
            try {
                const res = await fetch(`${API_BASE}/add-university-Staff`, {
    ...
    const validateForm = () => {
        ...
        if (selectedRole === 'CollegeAdmin') {
            const hasCollegeCode = String(formData.collegeCode || '').trim().length > 0;
            if (!hasCollegeCode) {
                alert('College Code is required for CollegeAdmin');
                return false;
            }
        }
        return true;
    ...
    const handleAddOrUpdate = async () => {
        ...
        if (selectedRole === 'CollegeAdmin') { payload.collegeCode = formData.collegeCode.trim(); }
        ...
        try {
            ...
            if (isEditing) {
                ...
                setEmployees(prev => prev.map(emp => emp.id === editingEmployeeId ? {
                    ...
                    collegeCode: data.staff.collegeCode || '',
                ...
                const newEmployee = {
                    ...
                    collegeCode: s.collegeCode || '', status: 'Active', joinDate: s.createdAt.slice(0, 10),
                };
    ...
    const stats = useMemo(() => ({
        ...
        governingBody: employees.filter(emp => emp.role === 'UniversityGoverningBody').length,
        registrar: employees.filter(emp => emp.role === 'UniversityRegistrar').length,
        examBody: employees.filter(emp => emp.role === 'UniversityExaminationBody').length,
        collegeAdmin: employees.filter(emp => emp.role === 'CollegeAdmin').length,
    }), [employees]);
                    ...
                    <div className="p-4 border-b border-gray-200">
                        <div className="flex flex-wrap gap-1">
                            {[
                                ...
                                { key: 'UniversityGoverningBody', label: 'Governing Body' }, { key: 'UniversityRegistrar', label: 'Registrar' },
                                { key: 'UniversityExaminationBody', label: 'Exam Body' }, { key: 'CollegeAdmin', label: 'College Admin' }
                            ].map(tab => (
                ...
                return (
                    <tr key={employee.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">
                            ...
                            {employee.collegeCode && <div className="text-xs text-gray-500 mt-1 font-mono">Code: {employee.collegeCode}</div>}
                        </td>
                        <td className="px-6 py-4">
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="p-6 space-y-6 overflow-y-auto">
                {!isEditing && (
                    <div><label className="block text-sm font-medium text-gray-700 mb-2">Employee Role *</label><select value={selectedRole} onChange={(e) => setSelectedRole(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"><option value="">Select role</option>{availableRoles.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}</select></div>
                )}
                {isEditing && (
                ...
                {selectedRole === 'CollegeAdmin' && (
                    <div><label className="block text-sm font-medium text-gray-700 mb-1">College Code *</label><input type="text" value={formData.collegeCode} onChange={(e) => setFormData({ ...formData, collegeCode: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Enter College Code" /></div>
                )}
            </div>
