...
// --- Helper Components (Toast and Loader for a self-contained file) ---
...
const ExamBodyResultsEntry = () => {
    ...
    // 2. Check for examId in URL to auto-select
    useEffect(() => {
        const params = new URLSearchParams(location.search);
        const examId = params.get('examId');
        if (examId && view === 'list' && exams.length > 0) {
            const examToSelect = exams.find(e => e._id === examId);
            if (examToSelect) {
                handleSelectExam(examToSelect);
            }
        }
    }, [location.search, view, exams]);


    // 3. Handle exam selection
    const handleSelectExam = async (exam) => {
        ...
        try {
            ...
            // FIX: Update the selectedExam with full details from the API response
    ...
    const handleSubmitResults = async () => {
        ...
        try {
            ...
            addToast('success', "Results have been successfully submitted for processing!");
            handleBack();
        } catch (err) {
            addToast('error', err.message);
        } finally {
    ...
    const handleBack = () => {
        ...
        setStudents([]);
        setMarks({});
        navigate('/university-exam-body/results-entry', { replace: true });
    };
...
const ResultsEntryGrid = ({ exam, students, marks, onMarkChange, onSubmit, onBack, isSubmitting, isLoading }) => {
    // FIX: Get subjects from the first student, but only if students exist.
    ...
    return (
        <div className="animate-fade-in">
            <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
                ...
                {/* FIX: Only show submit button if there are students to submit for */}
                {students.length > 0 && (
                    <button onClick={onSubmit} disabled={isSubmitting || isLoading} className="px-6 py-2.5 bg-emerald-600 text-white rounded-lg disabled:bg-emerald-300 flex items-center justify-center font-semibold hover:bg-emerald-700">
                        {isSubmitting ? <><Loader2 size={18} className="animate-spin mr-2" /> Submitting...</> : 'Submit & Process Results'}
                    </button>
                )}
            </div>
            ...
            {isLoading ? (
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-x-auto">
                    {/* FIX: Conditional rendering for the table vs. a "no students" message */}
                    {students.length > 0 ? (
