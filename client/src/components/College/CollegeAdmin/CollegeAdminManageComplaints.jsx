...
const TabButton = ({ label, count, active, onClick }) => (
  <button
    onClick={onClick}
    className={`flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg transition-colors ${active
        ? 'bg-indigo-100 text-indigo-700' // Corrected Active Style
        : 'text-slate-600 hover:bg-slate-100 hover:text-slate-800'

      }`}
  >
    {label}
    <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${active
        ? 'bg-indigo-200 text-indigo-800' // Corrected Active Style
