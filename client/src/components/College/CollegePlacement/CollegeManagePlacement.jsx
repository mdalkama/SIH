...
import { Loader2, PlusCircle, Eye, Edit, Users, X, Save, Link, Check, XCircle, Trash2, AlertTriangle } from 'lucide-react';
...
const Toast = ({ message, type, onClose }) => {
	...
	return (<div className={`fixed top-5 right-5 z-[100] flex items-center p-4 rounded-lg shadow-lg bg-white border-l-4 ${isError ? 'border-red-500' : 'border-green-500'}`}><div className={`text-lg ${isError ? 'text-red-500' : 'text-green-500'}`}>{isError ? <AlertTriangle /> : <Check />}</div><p className="ml-3 text-sm font-medium text-slate-800">{message}</p></div>);
};
		...
		<div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex justify-center items-center p-4">
			<div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
				<div className="p-4 border-b border-slate-300 flex justify-between items-center">
					<h2 className="text-lg font-bold text-slate-800">Student Applications</h2>
					<p className="text-sm text-slate-500">{drive?.jobTitle} at {drive?.companyName}</p>
					<button type="button" onClick={onClose} className="text-slate-400 hover:text-slate-600"><X /></button>
				</div>
				<div className="p-6 overflow-y-auto">{drive?.applications?.length > 0 ? (
					<table className="w-full text-sm">
						<thead className="text-left bg-slate-50 text-slate-600 text-xs uppercase">
							<tr><th className="p-3 font-semibold">Student Name</th>
								<th className="p-3 font-semibold">Registration No.</th>
								<th className="p-3 font-semibold text-center">Resume</th>
								<th className="p-3 font-semibold text-center">Status</th>
								<th className="p-3 font-semibold text-center">Actions</th>
							</tr>
						</thead>
						<tbody className="divide-y divide-slate-200">{drive.applications.map(app => (<tr key={app.studentId} className="hover:bg-slate-50">
							...
							<td className="p-3 text-center">
								<a href={app.resumeUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-blue-600 hover:underline font-semibold text-xs">
									<Link size={14} /> View Resume</a>
							</td>
							<td className="p-3 text-center">
								<span className={`px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(app.status)}`}>{app.status.replace('_', ' ')}</span>
							</td>
							<td className="p-3">
								<div className="flex justify-center gap-2">
									<button onClick={() => onStatusUpdate(drive._id, app.studentId, 'SHORTLISTED')} title="Shortlist" className="p-1.5 text-green-500 hover:bg-green-100 rounded-md">
										<Check size={16} />
									</button>
									<button onClick={() => onStatusUpdate(drive._id, app.studentId, 'REJECTED')} title="Reject" className="p-1.5 text-red-500 hover:bg-red-100 rounded-md">
										<XCircle size={16} />
									</button>
								</div>
							</td>
					...
					</table>) : (<p className="text-center p-8 text-slate-500">No applications have been received for this drive yet.</p>)}</div><div className="p-4 bg-slate-50 border-t border-slate-300 border-slate-300 flex justify-end"><button type="button" onClick={onClose} className="px-4 py-2 bg-slate-200 rounded-lg font-semibold hover:bg-slate-300 text-sm">Close</button></div></div></div>);
};
