...
const PostingOrdersPage = () => {
  ...
  const getOrderTypeBadge = (type) => {
    const typeClasses = {
      'Teaching': 'bg-blue-100 text-blue-800 :bg-blue-900/30 :text-blue-300',
      'Administrative': 'bg-purple-100 text-purple-800 :bg-purple-900/30 :text-purple-300',
      'Other': 'bg-gray-100 text-gray-800 :bg-gray-700 :text-gray-300'
    };
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="inline-flex rounded-md shadow-sm w-full md:w-auto">
            <button
              type="button"
              className={`px-4 py-2 text-sm font-medium rounded-l-lg ${activeTab === 'compassionate' ? 'bg-blue-600 text-white' : 'bg-white :bg-gray-700 text-gray-700 :text-gray-200 hover:bg-gray-50 :hover:bg-gray-600'}`}
            ...
            <button
              type="button"
              className={`px-4 py-2 text-sm font-medium rounded-r-lg ${activeTab === 'employee' ? 'bg-blue-600 text-white' : 'bg-white :bg-gray-700 text-gray-700 :text-gray-200 hover:bg-gray-50 :hover:bg-gray-600'}`}
                ...
                {activeTab === 'compassionate' ? (
                  filteredCompassionateOrders.length > 0 ? (
                    filteredCompassionateOrders.map((item, index) => (
                      <tr key={`compassionate-${index}`} className="hover:bg-gray-50 :hover:bg-gray-700/50 transition-colors">
                    ...
                    <tr>
                      <td colSpan="4" className="px-6 py-4 text-center text-sm text-gray-500 :text-gray-400">
                  ...
                  filteredEmployeeOrders.length > 0 ? (
                    filteredEmployeeOrders.map((item, index) => {
                      ...
                      return (
                        <tr key={`employee-${index}`} className="hover:bg-gray-50 :hover:bg-gray-700/50 transition-colors">
                          <td className="px-6 py-4 whitespace-normal">
                            <div className="flex items-center">
                              <FileText className="flex-shrink-0 h-5 w-5 text-blue-500 mr-3" />
                              <span className="text-blue-600 :text-blue-400 hover:underline font-medium">
