...
/**
 * Get the complete library profile for the currently logged-in student.
 */
export const getMyLibraryProfile = async (req, res) => {
  try {
    ...
    const student = await Student.findById(studentId).select("collegeCode").lean();
    if (!student || !student.collegeCode) {
        return res.status(404).json({ message: "Student or college code not found." });
    }
    ...
    const mainLibrary = await Library.findOne({ collegeCode: student.collegeCode }).lean();
    if (!mainLibrary) {
    ...
    const enrichedIssuedBooks = (studentLibrary.issuedBooks || []).map((book) => {
      const bookDetails = mainLibrary.books.find(b => b._id.equals(book.bookId));
      return { ...book, title: bookDetails?.title || "Unknown Book", author: bookDetails?.author || "N/A" };
    });

    const sortedActivity = (studentLibrary.activity || []).sort((a, b) => new Date(b.issuedAt) - new Date(a.issuedAt));
